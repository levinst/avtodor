@section('title', 'Информация для перевозчиков')

<section class="lg:max-w-7xl lg:mt-8 lg:mr-6">
    <div class="mb-20 text-center">
        <h1 class="text-3xl">Информация для перевозчиков</h1>
    ...
    @foreach ($carrier as $item)
        <div class="grid grid-cols-1 mb-6 md:grid-cols-[200px,1fr] gap-2 border-b lg:border-0">
            ...
            <div class="px-4 lg:px-2 ">
                ...
                <div class="mb-4 ">
                    <a href="{{ route('carrierView', $item['slug']) }}" class="font-semibold text-xl text-gray-600 hover:text-indigo-500 ">
                        {{ $item['title'] }}
                    </a>
                </div>
                <div>
                    {!! Str::words(strip_tags($item['text']), 25, ' ...') !!}
                    <a href="{{ route('carrierView', $item['slug']) }}" class="text-indigo-500 inline-flex items-center ml-2">
                        Подробнее
