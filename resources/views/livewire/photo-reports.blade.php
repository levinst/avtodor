@section('title', 'Фотоотчеты')

<section class="lg:max-w-7xl lg:mt-8 lg:mr-6">
    <div class="justify-center flex-1 px-4 text-left ">
        ...
        @foreach ($photoreports as $item)
            <div class="grid grid-cols-1 mb-6 md:grid-cols-[200px,1fr] gap-2 border-b lg:border-0">
                <div class="overflow-hidden h-40">
                    @if ($item['image'])
                        <img src="{{ route('home') }}/storage/images/photoreports/thumbs/{{ $item['image'] }}"
                ...
                <div class="px-4 lg:px-2 ">
                    <div>
                        <a href="{{ route('photoreportsView', $item['slug']) }}" class="font-semibold text-xl text-gray-600 hover:text-indigo-500 ">
                            {{ $item['title'] }}
                        </a>
                    </div>
                    <div class="text-xs mt-1 font-medium text-gray-600">
                        {{ $item['created_at']->format('d/m/Y') }}
                    </div>
                    <div class="w-8 pb-1 border-b border-gray-600"></div>
                    <div class="mt-4 ">
                        {!! Str::words(strip_tags($item['text']), 25, ' ...') !!}
                        <a href="{{ route('photoreportsView', $item['slug']) }}" class="text-indigo-500 inline-flex items-center ml-2">
                            Подробнее
