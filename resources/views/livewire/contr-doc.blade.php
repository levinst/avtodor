        ...
        <div class="grid grid-cols-1 mb-6 md:grid-cols-[200px,1fr] gap-2 border-b lg:border-0">
            ...
            <div class="px-4 lg:px-2 ">
                <div class="text-sm font-medium text-gray-600">
                    {{ $item['created_at']->format('d/m/Y') }} - {{ $item->contrcatdoc['title'] }}
                </div>
                <div class="w-8 pb-1 border-b border-gray-600"></div>
                <div class="mb-4 ">
                    <a href="{{ route('contrdocView', $item['slug']) }}" class="font-semibold text-xl text-gray-600 hover:text-indigo-500 ">
                        {{ $item['title'] }}
                    </a>
                </div>
                <div>
                    {!! Str::words(strip_tags($item['text']), 25, ' ...') !!}
                    <a href="{{ route('contrdocView', $item['slug']) }}" class="text-indigo-500 inline-flex items-center ml-2">
                        Подробнее
